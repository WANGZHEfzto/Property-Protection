  ...
  <div class="adminDetail-detailcontainer searchInput">
      ...
      <div class="adminDetail-main">
        <div class="breadcrumb">
            ...
            <el-breadcrumb-item>{{type==='1'?'侵权判定咨询':type==='2'?'维权援助':'纠纷调解'}} 待审核</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="main">
            <el-form ref="form" :model="formInfo" label-width="110px" v-loading="loading">
                ...
                <el-form-item label="被诉产品链接" class="row" v-if="type==='1'">
                    <!-- <el-input v-html="item" placeholder="请输入链接" :disabled="isDisabled" v-for="(item,i) in bsljList" :key="i"></el-input> -->
            ...
            <el-form ref="form1" :model="submitInfo" label-width="110px" :rules="formRules">
                ...
                <el-form-item label="审核人" class="half-row" prop="shr1">
                    <el-select v-model="submitInfo.shr1">
                        <el-option label="张三" value="张三"></el-option>
                        <el-option label="李四" value="李四"></el-option>
                        <el-option label="王五" value="王五"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审核时间" class="half-row" prop="shsj">
                    <!-- <el-input v-model="submitInfo.shsj1"></el-input> -->
                    <el-date-picker type="date" placeholder="选择日期" value-format="yyyyMMdd" v-model="submitInfo.shsj" style="width: 100%;"></el-date-picker>
                </el-form-item>
                <el-form-item label="审核结果" class="half-row" prop="shjg">
                    <el-select v-model="submitInfo.shjg">
                ...
                <!-- <el-form-item label="审核理由" class="row">
                    <el-input type="textarea" autosize v-model="submitInfo.shly"></el-input>
                </el-form-item> -->
                <el-form-item label="说明" class="row" prop="sm">
                    <el-input type="textarea" autosize v-model="submitInfo.sm"></el-input>
                </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        shr1: [{ required: true, message: '请选择', trigger: 'change' }],
        shsj: [{ required: true, message: '请选择', trigger: 'change' }],
        shjg: [{ required: true, message: '请选择', trigger: 'change' }],
        sm: [{ required: true, message: '请输入', trigger: 'blur' }]
      },
      isDisabled: true,
      loading: false,
      submitInfo: {
  ...
  methods: {
    submitForm(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          this.submitInfo.shzt = this.type === '1' ? '1' : '2'
          if (this.submitInfo.shjg === '0') {
            this.submitInfo.shzt = '2'
          }
          this.submitInfo.id = this.formInfo.id
          updateInfo(url, this.submitInfo, { wjids: this.dshIds, deleteWjids: '' }).then(res => {
            if (res.success === true) {
              this.$message({
                type: 'success',
                message: '提交成功！'
    ...
    onRemove1(file, fileList) {
      var ids = []
      ids = this.dshIds.split(',')
      var i = ids.indexOf(file.id + '')
      ids.splice(i, 1)
      this.dshIds = ids.join(',')
    }
  }
}
</script>
